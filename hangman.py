...
of finding vowels and consonants is 2:1. 
-Sriparna Majumdar
TA
...
def wordLetter(file):
    if not file.startswith("http"):
        with open(file, 'rt') as datafile:
            words = set([word.strip(punc) for word in datafile.split() if (len(word.strip(punc)) <= 10 
                                                                                 and len(word.strip(punc)) >= 2)])
            words_few = [word for word in words if re.search(r'[aeiou]+', word)]
    else:
        datafile = urlopen("http://hills.ccsf.edu/~abrick/urantia.txt").read().decode()
        words_few = [word.strip(punc) for word in set(datafile.split()) \
			if (word.strip(punc).isalpha() and len(word.strip(punc)) >= 2 and len(word.strip(punc)) <= 10)]
